...
class NLInterpreterAgent:
    ...
    def __init__(self):
        ...
        self.cross_generation = bool(os.environ.get("GENERATION_PROVIDER"))
        if not self.cross_generation and not openai_client:
            raise RuntimeError("OpenAI client is not configured")
        # ============ 클라이언트 저장 ============
        # OpenAI 클라이언트를 인스턴스 변수로 저장합니다
        self.client = openai_client
        self.llm = make_generation_chat("openai") if self.cross_generation else None
        # ============ 다음 에이전트 엔드포인트 설정 ============
...
"""
        ...
        if self.llm is not None:
            ...
            resp = await self.client.responses.create(
                ...
                text={"verbosity": os.environ.get("OPENAI_VERBOSITY", "low")},
                max_output_tokens=int(os.environ.get("INTERPRETER_MAX_OUTPUT_TOKENS", "300")),
            )
